...
void exclusionLimit(paramList *pL, int detj)
{
    ...
    if(pL->elecScat)
        sprintf(filename, "%sexclusionE_%s_%s_BSM%d.dat",pL->root,pL->detectors[0].name,pL->sources[pL->detectors[pL->detj].sourcej].name,pL->BSM);
    else
        sprintf(filename, "%sexclusionN_%s_%s_BSM%d.dat",pL->root,pL->detectors[0].name,pL->sources[pL->detectors[pL->detj].sourcej].name,pL->BSM);
