...
int calcRates(paramList *pList)
{
    ...
    for(int detj=0; detj < pList->ndet; detj++)
    {
        //open file
        if(pList->nucScat)
            sprintf(filename,"%s%sRateN_%s_%c%c_m%dkeV.dat", pList->root, BSMname, pList->detectors[detj].name, pList->sources[pList->detectors[detj].sourcej].name[0], pList->sources[pList->detectors[detj].sourcej].name[1],masskeV);
        else
            sprintf(filename,"%s%sRateE_%s_%c%c_m%dkeV.dat", pList->root, BSMname, pList->detectors[detj].name, pList->sources[pList->detectors[detj].sourcej].name[0], pList->sources[pList->detectors[detj].sourcej].name[1],masskeV);
...
int calcRatesThreshold(paramList *pList)
{
    ...
    for(int detj=0; detj < pList->ndet; detj++)
    {
        //open file
        if(pList->nucScat)
            sprintf(filename,"%s%sRateThN_%s_%c%c_m%dkeV.dat", pList->root, BSMname, pList->detectors[detj].name, pList->sources[pList->detectors[detj].sourcej].name[0], pList->sources[pList->detectors[detj].sourcej].name[1],masskeV);
        else
            sprintf(filename,"%s%sRateThE_%s_%c%c_m%dkeV.dat", pList->root, BSMname, pList->detectors[detj].name, pList->sources[pList->detectors[detj].sourcej].name[0], pList->sources[pList->detectors[detj].sourcej].name[1], masskeV);
        outfile.open(filename,std::ios::out);
