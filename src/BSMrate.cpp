...
double BSMrateE(double ErKeV, paramList *pList, double Mt, int sourcej, int fluxj)						  
{
    ...
    intConst	= fluxIntegral( ErGeV, pList, ME,  0, sourcej, fluxj);  //units GeV^2/s
    intInvEnuSq = fluxIntegral( ErGeV, pList, ME, -2, sourcej, fluxj); //units 1/s
...
double BSMrateN(double ErKeV, paramList *pList, double Mt, int sourcej, int fluxj)						  
{
    ...
    intConst	= fluxIntegral( ErGeV, pList, Mt,  0, sourcej, fluxj); //units GeV^2/s
    intInvEnu   = fluxIntegral( ErGeV, pList, Mt, -1, sourcej, fluxj); //units GeV/s
    intInvEnuSq = fluxIntegral( ErGeV, pList, Mt, -2, sourcej, fluxj); //units 1/s
...
double BSMrate(double ErKeV, paramList *pList, int detj, int sourcej, int fluxj)
{
	...
	{   
    	...
    	if(pList->nucScat)
    	{
		    ...
		    rate += targetsPerKG * pList->detectors[detj].isoFrac[i] * BSMrateN( ErKeV, pList, MN*pList->detectors[detj].isoA[i], sourcej, fluxj);
	    }
	    if(pList->elecScat)
	    {
	        int Ne=0;
	        while(pList->detectors[detj].ionization[i][Ne] > ErKeV && Ne < pList->detectors[detj].isoZ[i]) 
	            Ne++;
            if(pList->sources[pList->detectors[detj].sourcej].isSolar[fluxj] == 1)
	        {
		        pList->qA = 0.5;
		        pList->qV = 2*SSW+0.5;
		        rate += pList->sources[pList->detectors[detj].sourcej].survProb[fluxj] * ((double) pList->detectors[detj].isoZ[i] - Ne) * targetsPerKG * pList->detectors[detj].isoFrac[i] * BSMrateE( ErKeV, pList, ME, sourcej, fluxj);
		        
		        pList->qA = -0.5;
		        pList->qV = 2*SSW-0.5;
                rate += (1-pList->sources[pList->detectors[detj].sourcej].survProb[fluxj]) * ((double) pList->detectors[detj].isoZ[i] - Ne) * targetsPerKG * pList->detectors[detj].isoFrac[i] * BSMrateE( ErKeV, pList, ME, sourcej, fluxj);		     
	        }
		    else
		    {
		        //only include if doing reactor neutrinos (anti-neutrino channel)
		        if(pList->sources[pList->detectors[detj].sourcej].isSolar[0] == 0)
		        {
		            pList->qA = -0.5;
		            pList->qV = 2*SSW+0.5;
                    rate += ((double) pList->detectors[detj].isoZ[i] - Ne) * targetsPerKG * pList->detectors[detj].isoFrac[i] * BSMrateE( ErKeV, pList, ME, sourcej, fluxj);
                }
		    }
...
{   
    double rate=0;
    for(int i=0; i< pList->sources[pList->detectors[detj].sourcej].numFlux; i++)
    {
        if(pList->BSM==3 || pList->BSM==4)
            rate += pList->sources[pList->detectors[detj].sourcej].nuFluxNorm[i] * ( signalNorm * gsl_spline_eval(pList->detectors[detj].signalBSM1[i], ErkeV, pList->detectors[detj].accelBSM1[i])
...
{   
    ...
    {
        if(pList->BSM==3 || pList->BSM==4)
            rate +=  pList->sources[pList->detectors[detj].sourcej].nuFluxNorm[i] * ( signalNorm * gsl_spline_eval_integ(pList->detectors[detj].signalBSM1[i], Er_min, Er_max, pList->detectors[detj].accelBSM1[i])
