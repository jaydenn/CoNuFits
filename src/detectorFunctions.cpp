...
double detBackground(double Er, paramList *pList, int detj, int sourcej, int fluxj)
{
...
int newDetector(paramList *pList, char *name, double exp, int sourcej)
{
        ...
        for(int fluxj=0; fluxj< pList->sources[pList->detectors[pList->detj].sourcej].numFlux; fluxj++)
        {
            pList->sources[pList->detectors[pList->detj].sourcej].nuFluxNorm[fluxj] = 1;
