...
double *confIntervalSM(paramList *pL)
{
    ...
    double muUp,muDown;
    double pVal = 0;
    double guess = 2 + 10*pL->sources[pL->detectors[pL->detj].sourcej].isSolar[0];
...
void confIntVsExposure(paramList *pL)
{
    ...
    if(pL->elecScat)
        sprintf(filename, "%sCLe_%s_%s_SM.dat",pL->root,pL->detectors[detj].name,pL->sources[pL->detectors[pL->detj].sourcej].name);
    else
        sprintf(filename, "%sCLn_%s_%s_SM.dat",pL->root,pL->detectors[detj].name,pL->sources[pL->detectors[pL->detj].sourcej].name);
...
double tempBG(double Er, paramList *pList, int detj, int sourcej, int fluxj)
{
    return TEMPBG;
}
...
void SMsignificanceExp(paramList *pList)
{
    ...
    for (int i=0; i<201; i++)
    {
        ...
        for (int j=0; j<201; j++)
        {
            pList->sources[pList->detectors[pList->detj].sourcej].nuFluxUn[0] = .01 * pow(stepk,j);
            pList->detectors[0].BgUn =  .01 * pow(stepj,j);

            //sigma = sqrt(q0SM(pList));
            sigma = MCsignificance(pList, 0.1);
            
            std::cout << TEMPBG << " " << pList->sources[pList->detectors[pList->detj].sourcej].nuFluxUn[0] << " " << pList->detectors[0].BgUn << " " << sigma << std::endl;
            outfile   << sigma  << "    ";
