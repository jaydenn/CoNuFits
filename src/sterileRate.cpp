...
double sterileRate(double ErKeV, paramList *pList, int detj, int sourcej, int fluxj)							  
{
    ...
    for(int i=0;i<pList->detectors[detj].nIso;i++)
	{
	    ...
	    if(pList->elecScat)
	    {
	        ...
	        if(pList->sources[sourcej].isSolar[fluxj] == 1)
	        {
		        ...
		        rate += pList->sources[sourcej].survProb[fluxj] * (pList->detectors[detj].isoZ[i]-Ne) * targetsPerKG * pList->detectors[detj].isoFrac[i] * nuRateOsc( ErKeV, &pListSM, ME, sourcej, fluxj);

		        pListSM.qA = -0.5;
		        pListSM.qV = 2*SSW-0.5;
		        rate += (1-pList->sources[sourcej].survProb[fluxj]) * (pList->detectors[detj].isoZ[i]-Ne) * targetsPerKG * pList->detectors[detj].isoFrac[i] * nuRateOsc( ErKeV, &pListSM, ME, sourcej, fluxj);
		    }
		    else
		    {
		        pListSM.qA = -0.5;
		        pListSM.qV = 0.5+2*SSW;
		        rate += (pList->detectors[detj].isoZ[i]-Ne) * targetsPerKG * pList->detectors[detj].isoFrac[i] * nuRateOsc( ErKeV, &pListSM, ME, sourcej, fluxj);
		    }
...
{   
    ...
    for(int i=0; i< pList->sources[pList->detectors[detj].sourcej].numFlux; i++)
    {
        if( ErkeV < (pList->detectors[detj].ErL + (double)999*(pList->detectors[detj].ErU-pList->detectors[detj].ErL)/900) )
            rate += pList->sources[pList->detectors[detj].sourcej].nuFluxNorm[i] * gsl_spline_eval(pList->detectors[detj].signalBSM1[i], ErkeV, pList->detectors[detj].accelBSM1[i]);
    }
    return rate;
}
...
{   
    double rate = 0;
    for(int i=0; i< pList->sources[pList->detectors[detj].sourcej].numFlux; i++)
        rate += pList->sources[pList->detectors[detj].sourcej].nuFluxNorm[i] * gsl_spline_eval_integ(pList->detectors[detj].signalBSM1[i], Er_min, Er_max, pList->detectors[detj].accelBSM1[i]);
