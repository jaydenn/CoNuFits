...
double my_LS(const gsl_vector *v, void *params)
{
    ...
    for(int i=0; i < pL->sources[pL->detectors[pL->detj].sourcej].numFlux; i++)
    {
        pL->sources[pL->detectors[pL->detj].sourcej].nuFluxNorm[i] = fabs(gsl_vector_get(v, i+2));
        l += 0.5/pow(pL->sources[pL->detectors[pL->detj].sourcej].nuFluxUn[i],2) * pow(pL->sources[pL->detectors[pL->detj].sourcej].nuFluxNorm[i] - 1,2);
    }
    l -= logLikelihood(pL);
...
double my_L0(const gsl_vector *v, void *params)
{
    ...
    for(int i=0; i < pL->sources[pL->detectors[pL->detj].sourcej].numFlux; i++)
    {
        pL->sources[pL->detectors[pL->detj].sourcej].nuFluxNorm[i] = fabs(gsl_vector_get(v, i+1));
        l += 0.5/pow(pL->sources[pL->detectors[pL->detj].sourcej].nuFluxUn[i],2) * pow(pL->sources[pL->detectors[pL->detj].sourcej].nuFluxNorm[i] - 1,2);
    }
...
double findMaxLS(paramList *pL)
{
    ...
    gsl_vector *x,*dx;
    my_func.n = 2 + pL->sources[pL->detectors[pL->detj].sourcej].numFlux;
...
double findMaxL0(paramList *pL)
{
    ...
    //start point
    gsl_vector *x,*dx;
    my_func.n = 1 + pL->sources[pL->detectors[pL->detj].sourcej].numFlux;
...
{	 
    ...
    if(pL->asimov==1)
    {
        ...
        for(int fluxj=0; fluxj < pL->sources[pL->detectors[pL->detj].sourcej].numFlux; fluxj++)
            pL->sources[pL->detectors[pL->detj].sourcej].nuFluxNorm[fluxj] = 1.0;
...
double my_q0(const gsl_vector *v, void *params)
{
    ...
    for(int i=0; i < pL->sources[pL->detectors[pL->detj].sourcej].numFlux; i++)
        pL->sources[pL->detectors[pL->detj].sourcej].nuFluxNorm[i] = 1;
...
void discLimitVsMmed(paramList *pL, int detj)
{
    ...
    if(pL->elecScat)
        sprintf(filename, "%sDLe_%s_%s_BSM%d.dat",pL->root,pL->detectors[0].name,pL->sources[pL->detectors[pL->detj].sourcej].name,pL->BSM);
    else
        sprintf(filename, "%sDLn_%s_%s_BSM%d.dat",pL->root,pL->detectors[0].name,pL->sources[pL->detectors[pL->detj].sourcej].name,pL->BSM);
...
void discLimitEvolution(paramList *pL, int detj)
{
    ...
    if(pL->elecScat)
        sprintf(filename, "%sdiscEvoE_%c%c_%c%c_BSM%d.dat",pL->root,pL->detectors[0].name[0],pL->detectors[0].name[1],pL->sources[pL->detectors[pL->detj].sourcej].name[0],pL->sources[pL->detectors[pL->detj].sourcej].name[1],pL->BSM);
    else
        sprintf(filename, "%sdiscEvoE_%c%c_%c%c_BSM%d.dat",pL->root,pL->detectors[0].name[0],pL->detectors[0].name[1],pL->sources[pL->detectors[pL->detj].sourcej].name[0],pL->sources[pL->detectors[pL->detj].sourcej].name[1],pL->BSM);
...
void discLimitVsThresh(paramList *pL, int detj)
{
    ...
    if(pL->elecScat)
        sprintf(filename, "%sDLTe_%s_%s_BSM%d.dat",pL->root,pL->detectors[detj].name,pL->sources[pL->detectors[pL->detj].sourcej].name,pL->BSM);
    else
        sprintf(filename, "%sDLTn_%s_%s_BSM%d.dat",pL->root,pL->detectors[detj].name,pL->sources[pL->detectors[pL->detj].sourcej].name,pL->BSM);
